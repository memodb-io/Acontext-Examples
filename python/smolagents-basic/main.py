...
class ToolInvocationLogger:
    ...
    def log(self, tool_name: str, parameters: Dict[str, Any], result: Any):
        ...
        try:
            structured_result = json.loads(result) if isinstance(result, str) else {"result": result}
    ...
    for idx, invocation in enumerate(_logger.invocations, start=1):
        ...
        tool_calls.append({
            ...
            "function": {
                ...
                "arguments": json.dumps(invocation["parameters"], ensure_ascii=False),
            },
        })
        ...
        tool_responses.append({
            ...
            "content": json.dumps(invocation["structured_result"], ensure_ascii=False),
        })
...
def extract_messages_from_memory_steps(
    ...
    for step in new_steps:
        if isinstance(step, TaskStep):
            ...
            if not tool_calls_list and step.model_output_message and hasattr(step.model_output_message, 'tool_calls'):
                ...
                if step.observations:
                    ...
                    for idx, tc in enumerate(tool_calls):
                        messages.append({
                            "role": "tool",
                            "tool_call_id": tc["id"],
                            "content": observations_list[idx] if idx < len(observations_list) else ""
                        })
...
def send_message(client: "AcontextClient", message: Dict[str, Any], session_id: str) -> None:
    """Send a message to Acontext."""
    client.sessions.send_message(session_id=session_id, blob=message, format="openai")


def session_1(agent: ToolCallingAgent, client: "AcontextClient", session_id: str) -> None:
    ...
    final_answer, messages = run_agent_and_build_messages(agent, user_msg_1_content, reset=True)
    print(final_answer)
    ...
    final_answer, messages = run_agent_and_build_messages(agent, user_msg_2_content, reset=False)
    print(final_answer)
    all_messages.extend(messages)
    ...
    final_answer, messages = run_agent_and_build_messages(agent, user_msg_3_content, reset=False)
    print(final_answer)
    all_messages.extend(messages)
    ...
    final_answer, messages = run_agent_and_build_messages(agent, user_msg_4_content, reset=False)
    print(final_answer)
    all_messages.extend(messages)
    ...
    for task in tasks_response.items:
        print(f"\nTask #{task.order}:")
        print(f"  ID: {task.id}")
        print(f"  Title: {task.data.get('task_description', 'N/A')}")
    ...
    for msg in messages:
        ...
        if role == "user":
            ...
            if tool_calls:
                ...
                conversation_parts.append(f"Assistant: [Called tools: {', '.join(tool_info)}]")
...
def session_2(agent: ToolCallingAgent, client: "AcontextClient", session_id: str) -> None:
